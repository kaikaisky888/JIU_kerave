    ...
    var admin = {
        ...
        table: {
            render: function (options) {
                ...
                options.defaultToolbar = (options.defaultToolbar) ? options.defaultToolbar : ['filter', 'print', 'exports', {
                    title: '搜索',
                ...
                if (admin.checkMobile()) {
                    options.defaultToolbar = (options.defaultToolbar) ? options.defaultToolbar : ['filter', {
                        title: '搜索',
                        layEvent: 'TABLE_SEARCH',
                        icon: 'layui-icon-search',
                        extend: 'data-table-id="' + options.id + '"'
            ...
            buildToolbarHtml: function (toolbar, tableId) {
                ...
                toolbar.msgbefore = toolbar.msgbefore || true;
                ...
                formatToolbar.msgbefore = toolbar.msgbefore == true ? ' data-msgbefore="true" ' : '';
                formatToolbar.tableId = tableId !== undefined ? ' data-table="' + tableId + '" ' : '';
                html = '<button ' + formatToolbar.class + formatToolbar.method + formatToolbar.extend + formatToolbar.checkbox + formatToolbar.msgbefore + formatToolbar.tableId + '>' + formatToolbar.icon + formatToolbar.text + '</button>';
            ...
            buildOperatHtml: function (operat) {
                ...
                operat.msgbefore = operat.msgbefore || true;
                ...
                formatOperat.msgbefore = formatOperat.msgbefore ? ' data-msgbefore="true" ' : '';
                html = '<a ' + formatOperat.class + formatOperat.method + formatOperat.msgbefore + formatOperat.extend + '>' + formatOperat.icon + formatOperat.text + '</a>';
            ...
            image: function (data, option) {
                ...
                option.imageHeight = option.imageHeight || 20;
        ...
        listen: function (preposeCallback, ok, no, ex) {
            ...
            $('body').on('click', '[data-request]', function () {
                var title = $(this).attr('data-title'),
                    ...
                    checkbox = $(this).attr('data-checkbox'),
                    msgbefore = $(this).attr('data-msgbefore'),
             ...
             $('body').on('click', '[data-table-export]', function () {
                ...
                var index = admin.msg.confirm('根据查询进行导出，确定导出？', function () {
                    ...
                    var itemForm = $('button[lay-filter="currentTableRenderId_filter"]').parents().eq(1);
                    ...
                    window.location = admin.url(url)+"?filter="+JSON.stringify(formatFilter)+"&op="+JSON.stringify(formatOp);
                    layer.close(index);
                });
            });
            // // excel导出
        ...
        api: {
            ...
            upload: function () {
                ...
                var uploadSelectLists = document.querySelectorAll("[data-upload-name]");

                if (uploadList.length > 0) {
                    $.each(uploadList, function (i, v) {
                        var exts = $(this).attr('data-upload-exts'),
                            uploadName = $(this).attr('data-upload'),
                            uploadNames = $(this).attr('data-uploads'),
                        ...
                        $(elem).bind("input propertychange", function (event) {
                            ...
                            if(uploadNames){
                                $('#bing-' + uploadNames).remove();
                            }
                            if (urlString.length > 0) {
                                var parant = $(this).parent('div');
                                var liHtml = '';
                                $.each(urlArray, function (i, v) {
                                    liHtml += '<li><a><img src="' + v + '" data-image  onerror="this.src=\'' + BASE_URL + 'admin/images/upload-icons/' + uploadIcon + '.png\';this.onerror=null"></a><small class="uploads-delete-tip bg-red badge" data-upload-delete="' + uploadName + '" data-upload-deletes="' + uploadNames + '" data-upload-url="' + v + '" data-upload-sign="' + uploadSign + '">×</small></li>\n';
                                });
                    ...
                    $('body').on('click', '[data-upload-delete]', function () {
                        var uploadName = $(this).attr('data-upload-delete'),
                            uploadNames = $(this).attr('data-upload-deletes'),
                ...
                //扩展业务员选择
                if (uploadSelectLists.length > 0) {
                    $.each(uploadSelectLists, function (i, v) {
                        var uploadName = $(this).attr('data-upload-name'),
                            uploadNumber = $(this).attr('data-upload-number');
                        uploadNumber = uploadNumber || 'one';
                        var selectCheck = uploadNumber === 'one' ? 'radio' : 'checkbox';
                        var elem = "input[name='" + uploadName + "']",
                        ...
                        tableSelect.render({
                            ...
                            searchType: 'more',
                            searchList: [
                                {searchKey: 'title', searchPlaceholder: '请输入业务员帐号'},
                            ],
                            table: {
                                url: admin.url('ajax/getAllfronts'),
                                cols: [[
                                    {type: selectCheck},
                                    {field: 'id', title: 'ID'},
                                    {field: 'username', minWidth: 150, title: '用户信息', align: "center"},
                                    {field: 'level_name', width: 150, title: '上级', align: "center"},
                                    {field: 'holder_name', width: 120, title: '股东', align: "center"},
                                    {field: 'create_time', width: 200, title: '创建时间', align: "center", search: 'range'},
                                ]]
                            },
                            done: function (e, data) {
                                var urlArray = [];
                                $.each(data.data, function (index, val) {
                                    urlArray.push(val.username)
                                });
                                var url = urlArray.join('');
                                admin.msg.success('选择业务员成功', function () {
                                    $(elem).val(url);
                                    $(elem).trigger("input");
                                });
                            }
