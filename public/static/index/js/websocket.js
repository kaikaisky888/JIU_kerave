	...
	message(resp) {
		...
		if(msg.type == "allticker"){
			...
			for (var i = 0; i < ticker.length; i++) {
				...
				if(page_out=="home"){
					if(change > 0){
						$("#change_"+ticker[i].market).removeClass("color-red").addClass("color-green");
						$("#change_"+ticker[i].market).html('+'+change+'%');
					}else{
						$("#change_"+ticker[i].market).removeClass("color-green").addClass("color-red");
						$("#change_"+ticker[i].market).html(change+'%');
					}
		...
		if(page_out=="deal" || page_out=="leverdeal"){
			if(msg.depthlist && msg.market == localsym) {
				...
				for (var i = 0; i < bids.length; i++) {
					...
					if(i < depthnum){
						htmlbids +='<tr class="bgbgs" style="background-size: '+bwt+'% 100%;"><td>'+depth_bid+'</td><td class="color-green">'+bprice+'</td><td>'+btotal+'</td></tr>'
					}
				}
				...
				for (var i = 0; i < asks.length; i++) {
					if(asks[i]['price'] > 1){
						price = parseFloat(asks[i]['price']).toFixed(6)
					}else{
					...
					if(i < depthnum){
						htmlasks +='<tr class="bgbg" style="background-size: '+wt+'% 100%;"><td>'+depth_ask+'</td><td class="color-red">'+price+'</td><td>'+total+'</td></tr>'
					}
				}
