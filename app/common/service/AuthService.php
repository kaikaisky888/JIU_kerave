...
class AuthService
{
     ...
     * @var array
     */
    protected $config = ['auth_on' => true, 'system_admin' => 'system_admin', 'system_auth' => 'system_auth', 'system_node' => 'system_node', 'system_auth_node' => 'system_auth_node'];
    /**
     * 管理员信息
    ...
    {
        $nodeList = [];
        $adminInfo = Db::name($this->config['system_admin'])->where(['id' => $this->adminId, 'status' => 1])->find();
        if (!empty($adminInfo) && !empty($adminInfo['auth_ids'])) {
            $buildAuthSql = Db::name($this->config['system_auth'])->distinct(true)->whereIn('id', $adminInfo['auth_ids'])->field('id')->buildSql(true);
            $buildAuthNodeSql = Db::name($this->config['system_auth_node'])->distinct(true)->where("auth_id IN {$buildAuthSql}")->field('node_id')->buildSql(true);
            $nodeList = Db::name($this->config['system_node'])->distinct(true)->where("id IN {$buildAuthNodeSql}")->column('node');
        }
        return $nodeList;
    }
    /**
    ...
    {
        return Db::name($this->config['system_node'])->column('id,node,title,type,is_auth', 'node');
    }
    /**
     * 获取管理员信息
     * @time 2021-01-07
    ...
    {
        return Db::name($this->config['system_admin'])->where('id', $this->adminId)->find();
    }
    /**
     * 驼峰转下划线规则
     * @param string $node
