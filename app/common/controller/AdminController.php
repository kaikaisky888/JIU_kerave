...
class AdminController extends BaseController
{
    ...
    {
        parent::initialize();
        $this->lang_list = \think\facade\Config::get('lang.allow_lang_list');
        $this->layout && $this->app->view->engine()->layout($this->layout);
        $this->isDemo = Env::get('ffadmin.is_demo', false);
        $adminId = session('admin.id');
        if($adminId){
            $authService = app(AuthService::class);
            $this->adminInfo = $authService->getAdminInfo();
        }
    }
     ...
     * @return array
     */
    protected function buildTableParames($excludeFields = [], $admin_ids=null,$table_name=null)
    {
        ...
        foreach ($filters as $key => $val) {
            ...
            switch (strtolower($op)) {
                ...
                case 'range':
                    ...
                    $where[] = [$key, '>=', strtotime($beginTime. ' 00:00:00')];
                    $where[] = [$key, '<=', strtotime($endTime. ' 23:59:59')];
                    break;
                default:
                    $where[] = [$key, $op, "%{$val}"];
            }
        ...
        if($admin_ids){
            if($table_name){
                $key = $table_name.".level_ids";
            }else{
                $key = "{$tableName}.level_ids";
            }
            $where[] = ['','EXP',Db::raw("FIND_IN_SET(".$admin_ids.",".$key.")")];
        }
