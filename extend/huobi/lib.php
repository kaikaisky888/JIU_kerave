...
class req {
	...
	*/
	...
	function market_history($code) {
		$this->api_method = "/market/history/kline?symbol=".$code."&period=1min&size=2";
		$this->req_method = 'GET';
		return $this->curl($this->url . $this->api_method);
	}
	function market_merged($code) {
		echo nl2br("---------获取聚合行情-----------------\n");
		$this->api_method = "/market/detail/merged?symbol=".$code;
		$this->req_method = 'GET';
		return $this->curl($this->url . $this->api_method);
	}
	function market_trade($code) {
		echo nl2br("---------获取市场最近成交记录-----------------\n");
		$this->api_method = "/market/trade?symbol=".$code."&size=2";
		$this->req_method = 'GET';
		return $this->curl($this->url . $this->api_method);
	}
	function market_history_trade($code) {
		echo nl2br("---------批量获取获取最近的交易记录-----------------\n");
		$this->api_method = "/market/history/trade?symbol=".$code."&size=100";
		$this->req_method = 'GET';
		return $this->curl($this->url . $this->api_method);
	}
